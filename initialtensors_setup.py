"""A setup module for interfacing initialtensors.py with datadispenser.py."""
...
parinfo = {
    # Generic parameters
    "model": {"default": "", "idfunc": lambda dataname, pars: True},
    "dtype": {"default": np.float_, "idfunc": lambda dataname, pars: True},
    "initial2x2": {
        "default": False,
    ...
    "verbosity": {"default": 1, "idfunc": lambda dataname, pars: False},
    ...
    "h_trans": {
        "default": 1.0,
        "idfunc": lambda dataname, pars: ("qising" in pars["model"]),
    },
    "h_long": {
        "default": 0.0,
        "idfunc": lambda dataname, pars: ("qising" in pars["model"]),
    ...
    "tricrit_perturbation_factor": {
        "default": 0.1,
        "idfunc": lambda dataname, pars: ("qising_tricrit" in pars["model"]),
    },
    # Impurity parameters
    "impurity": {
        "default": None,
...
def contract2x2x2(A_list):
    """ Takes an iterable of rank 6 tensors and contracts a cube made of them
    to a single rank 6 tensor. If only a single tensor is given 8 copies of the
    same tensor are used.
    """
    if isinstance(A_list, (np.ndarray, TensorCommon)):
        A = A_list
        A_list = [A] * 8
...
def generate_A(*args, pars=dict()):
    ...
    if pars["initial4x4"]:
        ...
        join_dirs = (
            ...
            else [orig_dirs[0], orig_dirs[2], orig_dirs[4], orig_dirs[5]]
        )
...
def generate_A_impure(*args, pars=dict()):
    ...
    if pars["initial4x4"] or pars["initial2x2"] or pars["initial2x2x2"]:
        msg = (
            "initial2x2, initial4x4 and initial2x2x2 unimplemented for"
            "initial impurities."
        )
        ...
        A_impure = A_impure.join_indices(
            [0, 1], [3, 4], [6, 7], [8, 9], dirs=join_dirs
        )

"""Module that gets exact data such as free energies and scaling dimensions
for known models. Most functions expect as arguments a dictionary called pars,
where one of the keys would be called "model" and the value for that would be
the name of the model. Other necessary things may be expected to be included in
the same dictionary, such as a (key, value) pair for the inverse temperature.

Supported at the moment are only 2D classical square-lattice models, namely the
Ising, 3-state Potts, and six-vertex models. The model names used for these are
"ising", "potts3" and "sixvertex".
"""
    ...
    if model_name == "ising":
        if pars["H"] != 0:
            raise NotImplementedError(
                "Critical beta of Ising model with " "H != 0 not implemented."
            )
...
potts3_138 = (
    13 / 8,
    (1, 1, 2, 3, 4, 6, 9, 12, 16, 22, 29, 38, 50, 64, 82, 105),
)
potts3_25 = (2 / 5, (1, 1, 1, 2, 3, 4, 6, 8, 11, 15, 20, 26, 35, 45, 58, 74))
potts3_140 = (
    1 / 40,
    (1, 1, 2, 3, 4, 6, 9, 12, 17, 23, 31, 41, 54, 70, 91, 117),
)
potts3_115 = (
    1 / 15,
    (1, 1, 2, 3, 5, 7, 10, 14, 20, 26, 36, 47, 63, 81, 106, 135),
)
potts3_2140 = (
    21 / 40,
    (1, 1, 2, 3, 5, 7, 10, 14, 20, 26, 36, 47, 63, 81, 106, 135),
...
def get_primary_data(maxi, pars, alpha, qnum=None, **kwargs):
    ...
    if modelname == "ising" and "KW" in pars and pars["KW"]:
        if qnum is None or qnum == 0:
            prima_pairs += ((ising_0, ising_116), (ising_12, ising_116))
        if qnum is None or qnum == 1:
            prima_pairs += ((ising_116, ising_0), (ising_116, ising_12))
        ...
        if qnum is None or qnum == 0:
            prima_pairs += ((ising_0, ising_0), (ising_12, ising_12))
        if qnum is None or qnum == 1:
        ...
        if qnum is None or qnum == 0:
            prima_pairs += ((ising_116, ising_116),)
        if qnum is None or qnum == 1:
            prima_pairs += ((ising_12, ising_0), (ising_0, ising_12))
        ...
        if qnum is None or qnum == 1 or qnum == 2:
            prima_pairs += ((potts3_115, potts3_115), (potts3_23, potts3_23))
    elif modelname == "potts3" and alpha and np.allclose(alpha, np.pi * 2 / 3):
        if qnum is None or qnum == 0:
            prima_pairs += ((potts3_115, potts3_115), (potts3_23, potts3_23))
        if qnum is None or qnum == 1:
            prima_pairs += (
                (potts3_25, potts3_115),
                (potts3_0, potts3_23),
    ...
    # Truncate and sort.
    truncated_triples = it.takewhile(
        lambda t: t[0] < maxi, sorted(zip(scaldims, spins, degs))
    )
...
def get_free_energy(pars, **kwargs):
    ...
    if model_name == "ising":
        ...
        f = sixvertex_exact_f(
            a=pars["sixvertex_a"], b=pars["sixvertex_b"], c=pars["sixvertex_c"]
        )
    return f
...
def ising_exact_f(beta, J, H):
    if H != 0:
        raise NotImplementedError(
            "Free energy of Ising model with H != 0 " "not implemented."
        )
...
def sixvertex_exact_f(a, b, c):
    raise NotImplementedError()
    # TODO This may not be correct at all, and the integration does not work.
    m = 0.5 * (a + b + c)
    if a < m and b < m and c < m:
        ...
        def integrand(x):
            res = (
                (np.sinh(2 * (mu + omega) * x) / np.sinh(np.pi * x))
                * (np.sinh((np.pi - 2 * mu) * x) / np.cosh(2 * mu * x))
                / (2 * x)
            )
            return res

        f = eps_1 - integrate.quad(integrand, -100, 100, points=[0])[0]
    else:
        raise NotImplementedError()
    return f
